...
import '../../data/models/khelafat_member_model.dart';

/// Khelafat Majlis Primary Member Application Screen
class KhelafatMemberFormScreen extends StatefulWidget {
  const KhelafatMemberFormScreen({super.key});

  @override
  State<KhelafatMemberFormScreen> createState() => _KhelafatMemberFormScreenState();
}

class _KhelafatMemberFormScreenState extends State<KhelafatMemberFormScreen> {
  ...
  List<KhelafatMemberModel> _savedMembers = [];
  ...
  Future<void> _loadMembers() async {
    ...
    if (jsonStr != null) {
      ...
      setState(() {
        _savedMembers = decoded.map((item) => KhelafatMemberModel.fromJson(item)).toList();
      });
    }
  }
  ...
  Future<void> _saveForm() async {
    ...
    if (!_acceptedPledge) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('অনুগ্রহ করে অঙ্গীকারনামায় সম্মত হন।'), backgroundColor: Colors.red),
      );
      return;
    }

    final newMember = KhelafatMemberModel(
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('প্রাথমিক সদস্য ফরম সফলভাবে জমা হয়েছে!'), backgroundColor: AppTheme.primaryColor),
      );
    }
  }
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  KhelafatMemberModel.officialPledgeText,
                                  style: const TextStyle(fontSize: 12, height: 1.5, color: Color(0xFFA7F3D0)),
                                ),
